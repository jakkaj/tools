---
name: plan-8-v2-merge
description: Analyze upstream changes from main and generate merge plan document. Domain-aware v2.
---
...
# Find commits touching domain definitions
git log ${ANCESTOR}..${TARGET} --oneline -- \"docs/domains/\"

# For each unique plan folder touched, extract:
# - Plan ordinal and slug
# - Merge date
# - Commit count
# - Domains affected (from docs/domains/ changes and file path analysis)
```
  ...
  \"upstream_plans\": [
    {
      ...
      \"days_ago\": 10,
      \"commit_count\": 15,
      \"domains_touched\": [\"billing\", \"_platform/data-access\"]
    },
    ...
  ],
  \"domains_changed\": [\"billing\", \"auth\"],
  \"new_domains_registered\": [],
  \"total_commits\": 47,
  \"plan_count\": 3
}
```
...
**Domains Affected**:
- [list domains touched by this plan — check `docs/domains/registry.md` and file paths against domain source trees]
- [note any domain.md updates, new domains registered, or contract changes]
...
| File | Your Change | Upstream Change | Auto-Merge? | Conflict Type | Domain |
|------|-------------|-----------------|-------------|---------------|--------|
| path/to/file.py | Added method X | Modified method Y | Yes | Complementary | auth |
| path/to/model.py | Added field | Modified same field | No | Contradictory | billing |
...
|------|--------|---------|-------------|------------------|
| ${PLAN_1} | ${DAYS} days ago | ${PURPOSE} | ${RISK_LEVEL} | ${DOMAINS} |
| ... | ... | ... | ... | ... |
...
- Auto-fire `/harness-3-retro --harvest` — this is the plan-completion reflection moment; the user gets a curated view of all friction encountered across the plan's lifetime. Filter scope: `--plan <slug>` derived from the branch/cwd.
