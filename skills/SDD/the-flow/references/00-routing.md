...
**Create + seed block** — define the path **once** (real runs use `docs/plans/<ord>-<slug>/`; a scratch/eval run redirects only `PLAN_DIR`/`FLOW_PATH`). **Route A (plan 040 / D1): the FULL 10-node seed is instantiated from the pre-authored template in ONE `create --template` call** — the seed is complete-at-create (the `research → plan → phase-1 → review-1 → ship` spine **plus** the 5 harness chores and pre-authored per-node `instructions[]`, all baked into the template), so there is **no per-node `add-node`, no last-to-first build-order wart, and NO create-time chore apply** (`create` stamps the whole seed atomically; later structural edits use the transactional `apply`, which resolves forward `--next` refs in one batch — `flight-plan-ops.md` §6). **There is no router/provisioned gate at create** — D1 reversed the 039 split: the chores are optional/recommended + skippable, so an un-harnessed repo simply never runs them (harmless), and even a weak model gets the whole spine + chores + instructions with **zero inference**:
...
TEMPLATE="<skill base>/references/flight-plan.template.json" # the FULL 10-node seed (research → plan → phase-1 → review-1 → ship + 5 chores + per-node instructions[])

# root + the complete 10-node seed in ONE call — the template carries `cursor` (→ nav.now) + the 5 spine nodes
...
# already the full 10-node starter with zero conditional logic and no router/provisioned gate. (Multiphase: the
...
- **Create-seed (Route A, plan 040 / D1)**: [`flight-plan.template.json`](./flight-plan.template.json) — the **full 10-node** `--template` seed instantiated at `create` (`research → plan → phase-1 → review-1 → ship` + the 5 harness chores + per-node `instructions[]`, all baked in; **no create-time gate**). The chore *shape* it bakes is owned by the `doctrine-parity:039` block ([`harness-seams.md`](./harness-seams.md), workshop 001 WS-2) — the template is one consumer of that single source of truth.
...
2. **Reveal/refine the future** — `insert-node` owns every edge splice, no hand edge-recomputation ([`flight-plan-ops.md`](./flight-plan-ops.md) § 4 spine-vs-excursion, § 6 build-order):
   - at the `plan` pass (plan-complete), the **plan-complete ADDITIVE EXPANDER** runs (Route A, plan 040 / D1): it builds the full `(phase-N → review-N)* → ship` spine. For N>1 phases it emits **ONE** `harness flow apply --ops -` batch that upserts `phase-2..N` + `review-2..N` after `phase-1` (each `review-k.next=["phase-(k+1)"]`, `review-N.next=["ship"]`) **and re-asserts the existing `review-1` with `next=["phase-2"]`** (an `upsert`, NOT an `mv` — its template default is `next=["ship"]`, so a multiphase spine must re-point it to `phase-2` or phase-2 is unreachable) — **purely additive, nothing relocates, no `mv`**. Each phase carries its own spine **`review-N`** node (deterministic id `review-N`, type `review`, **`zone: flight`**, interleaved `phase-N → review-N → next` so the rail reads `[ P1·review-1·P2·review-2… ]·ship`) **plus** its own **boot + observe + drain** trio (deterministic ids `boot-N`/`observe-N`/`retro-N`), materialised from the **single shared shape doctrine** — the `doctrine-parity:039` block in [`harness-seams.md`](./harness-seams.md) (workshop 001 WS-2), the **one source of truth** for every chore's `type`/`label`/`kind`/`importance`/anchor/`command`. The expander **carries no inline copy** of the chore shape — that hardcoded skill copy was the old §3b drift (deleted, plan 040): phase-1's review + trio comes from the template, phases 2..N's from the *same doctrine the template was built from*, so the spine is uniformly harnessed end-to-end. **No create-time gate and no expansion gate** (D1): the chores are skippable, so an un-harnessed repo simply never runs them — there is no "router installed AND provisioned" condition at either point. A **Simple (1-phase)** plan needs no expansion — it was complete at `create`. The expander is **byte-stable idempotent** (AC-03): every op is a deterministic-id `upsert`, so **re-running it on a complete spine writes nothing** — which is exactly what lets the every-entry **§ Reconcile the spine** pass invoke it safely on **each** entry (the *idempotent superset* that catches adopted / post-pass-edited / direct-jump-built plans the single reveal misses). *(`insert-node --type phase` remains valid for a one-off manual phase add; the expander is the batch that keeps multiphase + its chores consistent.)*

     **Worked idempotency example** — a 3-phase plan, expander fired twice on the same flow:
     ```
     run 1 — phase-1 + review-1 complete (review-1.next=["ship"]), phases 2–3 + their review/chores missing:
       upserts phase-2, review-2, phase-3, review-3, boot-2/observe-2/retro-2, boot-3/observe-3/retro-3
       + re-asserts review-1.next=["phase-2"], review-2.next=["phase-3"], review-3.next=["ship"]
