...
TEMPLATE="<skill base>/references/flight-plan.template.json" # the FULL 9-node seed (research → plan → phase-1 → ship + 5 chores + per-node instructions[])

# root + the complete 9-node seed in ONE call — the template carries `cursor` (→ nav.now) + the 4 spine nodes
...
# already the full 9-node starter with zero conditional logic and no router/provisioned gate. (Multiphase: the
...
- **Create-seed (Route A, plan 040 / D1)**: [`flight-plan.template.json`](./flight-plan.template.json) — the **full 9-node** `--template` seed instantiated at `create` (`research → plan → phase-1 → ship` + the 5 harness chores + per-node `instructions[]`, all baked in; **no create-time gate**). The chore *shape* it bakes is owned by the `doctrine-parity:039` block ([`harness-seams.md`](./harness-seams.md), workshop 001 WS-2) — the template is one consumer of that single source of truth.
...
2. **Reveal/refine the future** — `insert-node` owns every edge splice, no hand edge-recomputation ([`flight-plan-ops.md`](./flight-plan-ops.md) § 4 spine-vs-excursion, § 6 build-order):
   - at the `plan` pass (plan-complete), the **plan-complete ADDITIVE EXPANDER** runs (Route A, plan 040 / D1): for N>1 phases it emits **ONE** `harness flow apply --ops -` batch that splices `phase-2..N` after `phase-1` — **purely additive, nothing relocates, no `mv`**. Each new phase carries its own **boot + observe + drain** trio (deterministic ids `boot-N`/`observe-N`/`retro-N`), materialised from the **single shared shape doctrine** — the `doctrine-parity:039` block in [`harness-seams.md`](./harness-seams.md) (workshop 001 WS-2), the **one source of truth** for every chore's `type`/`label`/`kind`/`importance`/anchor/`command`. The expander **carries no inline copy** of the chore shape — that hardcoded skill copy was the old §3b drift (deleted, plan 040): phase-1's trio comes from the template, phases 2..N's from the *same doctrine the template was built from*, so the spine is uniformly harnessed end-to-end. **No create-time gate and no expansion gate** (D1): the chores are skippable, so an un-harnessed repo simply never runs them — there is no "router installed AND provisioned" condition at either point. A **Simple (1-phase)** plan needs no expansion — it was complete at `create`. The expander is **byte-stable idempotent** (AC-03): every op is a deterministic-id `upsert`, so **re-running it on a complete spine writes nothing** — which is exactly what lets the every-entry **§ Reconcile the spine** pass invoke it safely on **each** entry (the *idempotent superset* that catches adopted / post-pass-edited / direct-jump-built plans the single reveal misses). *(`insert-node --type phase` remains valid for a one-off manual phase add; the expander is the batch that keeps multiphase + its chores consistent.)*

     **Worked idempotency example** — a 3-phase plan, expander fired twice on the same flow:
     ```
     run 1 — phase-1 complete, phases 2–3 + their chores missing:
       upserts phase-2, phase-3, boot-2/observe-2/retro-2, boot-3/observe-3/retro-3  → 8 nodes added, ONE render write.
     run 2 — spine now complete (e.g. the every-entry reconcile pass re-fires it):
       every upsert resolves to a byte-identical existing node      → 0 ops applied, NOTHING written, no render.
     ```
