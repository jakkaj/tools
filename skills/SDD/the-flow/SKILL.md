...
- **Clean break (`E308`).** Pre-024 hand-cranked flows (a `the-flow.json` with no `provenance` block) are **not** migrated — the CLI returns `E308` (legacy-format) on read. That is an honest stop, not a bug: re-create with `harness flow create flight-plan --schema <skill base>/references/flight-plan.schema.json` (`<skill base>` = this skill's base dir, e.g. `~/.claude/skills/the-flow`; any prior `.md` stays as a static record).
