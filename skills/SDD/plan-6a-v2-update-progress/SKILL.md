...
7) (Retired) Plan-level flight plan update — this duty left SDD entirely.
   The standalone flight-plan skill (`plan-5b`) was removed, so there is no
   `<slug>.fltplan.md` to maintain. Progress lives in the task table and the
   execution log (Steps 4–5); the journey-level view (`the-flow.md`) is owned
   and regenerated by `/the-flow` itself, never by this skill. (Step number
   kept so callers' "Step 8" / "Step 9" references stay stable.)
...
**Why the companion debrief lives here**: 6a already owns the "last task in phase" branch (the phase-complete summary in the execution log + task table). Adding the conditional companion debrief (Step 9) means `/plan-6-v2-implement-phase-companion` gets full phase-end ceremony coverage for free with zero duplication.
