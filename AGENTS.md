...
│   └── agents.sh      # Syncs agent commands and MCP configs to Claude/OpenCode/Codex CLIs
├── agents/            # AI agent assets (commands + MCP source configs)
│   ├── commands/      # Markdown command definitions copied into each CLI
│   └── mcp/           # Canonical MCP server definitions (JSON)
...
- **Agent MCP Config**: Shared Serena MCP server config deployed to Codex CLI, Claude CLI, and OpenCode CLI (global + project files)
