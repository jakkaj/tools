...
**ONLY** use **Complexity Score (CS 1-5)** scoring (Surface Area + Integration + Data/State + Novelty + NFR + Testing, 0-2 each):
...
5) **Transform and expand** plan-3 tasks into the canonical tasks format:
   ...
   **Canonical tasks table layout** (all dossiers MUST follow this column order):
   ...
   - `CS` is the Complexity Score (1-5): CS-1 (trivial), CS-2 (small), CS-3 (medium), CS-4 (large), CS-5 (epic). Score each task using S,I,D,N,F,T factors (0-2 each).
   ...
   - `Notes` include contextual references (e.g., ADR IDs, Critical Finding refs).
...
5a) **Launch Pre-Implementation Audit subagent** to audit all files from the expanded task table:
   ...
   Use `subagent_type="Explore"` (has Glob/Grep/Read access).
   ...
   **Subagent Prompt**:
   ...
   3. **DUPLICATION CHECK** (Created files only):
      - Use grep and glob to search for existing implementations of similar functionality. Check for duplicate code or reusable components.
...
S4) Launch Pre-Implementation Audit subagent:
