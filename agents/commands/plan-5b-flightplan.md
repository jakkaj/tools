...
1. **Review existing handler structure** — understand the base patterns
   before extending them (`src/handlers/base.py`)
2. **Write failing tests for the new API** — TDD approach, define the
   contract before implementation (`tests/test_api.py` — new file)
3. **Implement the POST endpoint** — wire up validation, processing,
   and response formatting (`src/api/endpoint.py` — new file)
4. **Integration test** — verify end-to-end flow works with real data
...
4) Generate the Flight Plan:
   ...
   **## Route**:
   - Create a numbered list from the tasks table.
   - Each item: **Bold action phrase** — one-sentence explanation (`affected file(s)` in parentheses, noting "new file" for Created files).
