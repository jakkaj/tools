# Sample `the-flow.md` (flight view)

> Illustrative — what `the-flow` would write to `docs/plans/001-project-setup/the-flow.md` for chainglass `001-project-setup` (6 real phases). **Generated from [`the-flow.json`](./sample-the-flow.json)**. Snapshot: Phases 1–2 done, **Phase 3 in progress**. Every workshop is its **own node**; each stage where the user typed shows a **verbatim speech bubble**.

**Plan**: project-setup · **Mode**: Full · **Phases**: 6
**Rail**: `[the-flow] ◆─◆─◆─[◆─◆─◇─◇─◇─◇]─◇`   ·   **now**: Phase 3 (Next.js) · **next**: Phase 4 (CLI)
...
flowchart TD
    ...
    %% ── spine (vertical); companion WRAPS the build phases ──
    R[Research]:::done --> S[Spec]:::done --> PL[Plan]:::done
...
_Generated from `the-flow.json`. Each spine/excursion node links its artifacts (plan folder, tasks, `execution.log.md`, `reviews/*.md`) and carries a note (what & why); nodes the user spoke at hang a yellow bubble with their **exact words**. Workshops are shown individually (W1, W2) — never collapsed. Before `/plan-3`, P4–P6 + Merge were `assumed`; locking the plan flipped them `known`. If Phase 3's review fails, `fix loop?` flips `assumed → in_progress`._
