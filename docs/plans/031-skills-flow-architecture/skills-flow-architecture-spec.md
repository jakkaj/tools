...
13. **Whole-skill literal census (the one-edit claim, honestly)**: post-restructure, a census of `/the-flow <id>`-shaped strings across the entire skill finds literals **only** in the authorized classes the census records: (a) the Grammar definition, (b) banner-marked rendered views, (c) marker-exempt frozen quotations (`<!-- lint:allow-flow-commands -->` — added during the build when the byte-frozen State-write ownership section was found to quote a command), and (d) durable-state worked-example data (flight-plan template/schema `command:`/`driver:` fields — state stores *rendered* commands per the render-at-write-time rule); the count is recorded in the execution log. The Summary's claim is thereby scoped: one authoritative edit + banner-marked views refreshed separately (regeneration tooling stays deferred).
