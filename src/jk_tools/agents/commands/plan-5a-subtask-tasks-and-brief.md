...
|------|--------|--------|-------------|----------------|
| /abs/path/docs/api-spec.md | Modify | Pre-plan | — | keep-as-is |
| /abs/path/tests/fixtures/bulk_gen.py | Create | New | — | keep-as-is |
| /abs/path/tests/fixtures/bulk/*.json | Create | New | — | keep-as-is |
...
|----|-------------|-------------|---------------|-------|--------|
| AC (T003) | Validate bulk import payloads | api-spec.md → bulk_gen.py → fixtures | 3 | ST001,ST002,ST003 | ✅ Complete |

### Gaps Found
No gaps — all subtask acceptance criteria have complete file coverage.
...
4) Content expectations for `${SUBTASK_FILE}` (mirror plan-5 layout, scoped to subtask):
   ...
   - `## Flight Plan` section (generated by step 4a) containing the pre-implementation file audit:
     ...
     * If step 4a subagent returned no findings: include section with note "No findings — all files are new or have clean provenance"
     * If `reuse-existing` recommendations found: add prominent callout that subtask table may need revision
     * If HIGH compliance violations found: add note that these must be addressed before implementation proceeds
   - `## Requirements Traceability` section (generated by step 4b) verifying acceptance criteria have complete file coverage:
     ...
     * **Orphan Files**: Files in subtask tasks that don't map to any AC — only if orphans exist
     * **Flow Details**: Numbered file chains for ACs with gaps or complex paths
     * If step 4b subagent returned no gaps: include section with note "All acceptance criteria have complete file coverage"
     * Scope is narrower than plan-5: only ACs this subtask addresses, parent task files counted as covered
...
4a) **Launch Flight Plan subagent** to audit all files from the subtask's ST### task list:

   After step 4 defines the subtask content (including file paths), launch a single subagent to build the Flight Plan — the pre-implementation audit that verifies provenance, checks for duplication, and validates compliance.
   ...
   **Subagent Prompt**:
   "Build a Flight Plan for subtask [SUBTASK_SUMMARY] in [PHASE_TITLE] of plan [PLAN_PATH].

   Files to investigate (from subtask ST### task Absolute Path(s) + parent task files):
   [LIST EVERY UNIQUE FILE FROM THE SUBTASK TASK TABLE]
   ...
   3. **DUPLICATION CHECK** (Created files only):
      - Use `/code-concept-search "<concept>" --no-provenance` for each major concept
        ...
        concepts even when named differently. Uses FlowSpace when available,
   ...
   4. **COMPLIANCE CHECK** (all files):
      - If `docs/adr/` exists: scan ADRs for constraints affecting this file
      - If `docs/project-rules/rules.md` exists: check against project rules
   ...
   **Output format** — return these sections:
   ...
   - **Include subagent output** verbatim in the `## Flight Plan` section of the subtask dossier
   - If `reuse-existing` recommendations found: flag prominently — subtask table may need revision
   - If HIGH compliance violations found: note that implementation should address these first

4b) **Launch Requirements Flow subagent** to verify every acceptance criterion (scoped to this subtask) has complete file coverage:

   After the Flight Plan audits files (step 4a), verify that the subtask's task table doesn't have gaps — files that an AC needs but no task covers. This is scoped to the subtask: only ACs the subtask addresses are traced, and parent task files count as "already covered."
   ...
   **Subagent Prompt**:
   "Run requirements flow tracing for subtask [SUBTASK_SUMMARY] in [PHASE_TITLE] of plan [PLAN_PATH].

   Acceptance Criteria relevant to this subtask (from spec + parent task context):
   [LIST ACs THAT THIS SUBTASK ADDRESSES — from parent task description and spec]

   Task table files (from subtask ST### table + parent task files):
   [LIST ALL UNIQUE FILES FROM THE SUBTASK TASK TABLE AND PARENT TASK Absolute Path(s)]

   Full subtask task table (for cross-referencing):
   [PASTE THE ST### TASK TABLE WITH ID, Task, Absolute Path(s) COLUMNS]

   For each acceptance criterion this subtask addresses:
   ...
   4. Cross-reference against the subtask + parent task files — does every file appear?
   5. Flag gaps: files in the flow but missing from both subtask and parent task tables
   6. Flag orphan files: files in subtask tasks that don't map to any AC

   Walk the codebase like a human engineer — read files, follow imports, trace call chains.
   Parent task files are considered 'already covered' — only flag files missing from BOTH tables.
   Scope is narrower than a full phase trace — focus on what this subtask needs to deliver.
   ...
   - **Review subagent output for gaps**. If gaps are found:
     ...
     * For genuine gaps: **add new ST### tasks** to the subtask table to cover the missing files
     * For false positives: note why the file isn't needed in the Requirements Traceability section
     * **Do NOT proceed to step 5** until all gaps are resolved
   - If no gaps found: proceed with the Requirements Traceability section showing full coverage
   - Include subagent output in the `## Requirements Traceability` section of the subtask dossier
