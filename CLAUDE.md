...
The engineering-harness loop (**Boot → Backpressure → Observe → Retro → Improve**) is owned by the external **eng-harness family** (`AI-Substrate/harness-engineering`) — this repo carries **no harness skills of its own** (the local harness skill family was deleted in plan-029). The SDD pipeline reaches the harness through exactly **one door**: the **`/eng-harness-flow`** router, called at lifecycle-hook seams with context — `--hook pre-flight | pre-coding | post-coding | post-flight` (the 021 hooks; the older `--event session-start | post-spec | pre-implement | phase-end | plan-complete` names remain a permanent alias). **Seam orchestration is flow-owned and lives in one file — [`skills/SDD/the-flow/references/harness-seams.md`](./skills/SDD/the-flow/references/harness-seams.md)** (the seam map, two-layer detection, node-emission rule, per-phase retro lifecycle, and the versioned upstream contract); the stage sub-skills are **harness-blind** (plan-033). The router's child skills are private and never named in this repo; the router decides what (if anything) the harness does at each seam, and SDD narrates its `--json` envelope verbatim (`decision: route|redirect|noop|ambiguous`; boot verdicts `healthy / SLOW / UNHEALTHY / UNAVAILABLE`). Install: `npx skills@latest add AI-Substrate/harness-engineering -a claude-code -g -y`.
...
**Depth**: see [`docs/plans/023-difficulty-ledger-skill/`](./docs/plans/023-difficulty-ledger-skill/) for the original spec + 6 workshops + implementation plan, [`docs/plans/024-harness-nucleus/`](./docs/plans/024-harness-nucleus/) for the 6→3 loop-stage consolidation, [`docs/plans/029-eng-harness-switchover/`](./docs/plans/029-eng-harness-switchover/) for the switchover to the external family, and [`docs/plans/033-flow-owned-harness-seams/`](./docs/plans/033-flow-owned-harness-seams/) for the inversion that made the seams flow-owned (engine-offered per-phase beats; orchestration in `skills/SDD/the-flow/references/harness-seams.md`, sub-skills harness-blind).
...
The main SDD flow is **one skill**: `skills/SDD/the-flow/`, built to the **flow-architecture pattern** ([`docs/skills-pipeline/flow-architecture.md`](./docs/skills-pipeline/flow-architecture.md) — read it before editing). The structure: the dispatch `SKILL.md` owns the **Registry** (id ↔ verb ↔ module, the single master) + the **Command grammar** (defined exactly once) + the translation/alias table; `references/00-routing.md` owns the **Graph** (the single owner of "what's next") + state contract; `references/coach.md` is the narration voice (render slots, never literal commands); `references/harness-seams.md` is the **flow-owned home for harness-seam orchestration** (where/when each seam fires + the versioned upstream contract — sub-skills are harness-blind, plan-033); `references/getting-started.md` is a banner-marked **rendered view** (regenerate, never hand-edit); `references/stages/*.md` are the **sub-skills** — contract-bound verbs that know nothing about the flow (no stage ids, no successor names, no flow commands). To change stage behaviour, edit the **sub-skill**; to change routing, edit the **Graph**; to change where/when the harness fires, edit **`references/harness-seams.md`**; to change the command surface, edit the **grammar line** (one edit — that's the point). Lint any change with `just check-flow` (`scripts/check-flow-architecture.sh`) — hard-fail, deterministic. History: the 12 per-stage `plan-*` skills were consolidated on 2026-06-11 (plan-030, rollback tag `pre-flow-consolidation`); the sub-skill/Registry/Graph restructure + companion fold (6c → `implement --companion`, typed `6c` still alias-resolves) landed in plan-031 (`docs/plans/031-skills-flow-architecture/`). The two planning stages — `1b specify` and `3 architect` — were then merged into a single **atomic `1b plan` verb** (one `<slug>-plan.md`: a `## Business Specification` half on top, a `## Implementation Plan` half below, always both in one pass) in plan-032 (`docs/plans/032-unified-planning-doc/`); `20-specify.md` + `30-architect.md` became `20-plan.md`, and typed `specify`/`architect` (and id `3`) alias-resolve to `1b plan`. The 10 utility skills under `skills/SDD/` are ordinary standalone skills — edit their `SKILL.md` directly. The old `scripts/migrate-skills.py` (which generated the v2 skills from the long-gone `agents/v2-commands/`) was deleted in plan-030 — git history retains it.

### Standing requirement — the flight plan must always represent the whole spine

**The flight plan (`the-flow.json` → rendered `the-flow.md`) is the SPINE — the legible record of the whole journey — and it MUST always be complete without the user having to ask.** On **every** guided entry (`/the-flow` resume *and* adopt), the engine must **reconcile the spine declaratively**, not just build it reactively on the happy path:

- **All phases and workshops — past, present, and future** — that the plan (`<slug>-plan.md` `### Phase Index`) and any `workshops/*.md` imply must appear as nodes in `the-flow.json`. Reveal-at-the-`plan`-pass is not enough: adoption from existing artifacts, a plan edited after the pass, and direct-jump-built plans all leave the spine incomplete, and the next guided entry must **diff the full roster against the nodes and backfill what's missing**.
- **All phases and chores from `eng-harness-flow`** — boot, backpressure, observe reminders, retro-drain, harvest — must be anchored across the known phases (when the router is installed + the repo provisioned), so the whole loop is visible up front rather than appearing only as you reach each edge. the-flow emits the seam **nodes** (`harness-boot` / `backpressure` / `harness-retro`); `eng-harness-flow` owns the chore **flag** layered on top (`references/harness-seams.md` § Chore-flag ownership) — reconcile must respect that ownership split and dedup on the `--hook` token, never double-place.

This reconciliation is **idempotent, advisory, best-effort, and CLI-driven** (all mutation through `harness flow`; a complete spine is a no-op; never gates/blocks; no new state file — the "what should be here" view is recomputed from the plan artifact at read time per the KISS doctrine). **Where it lives**: the reconcile step + its per-entry trigger belong in `references/00-routing.md` (the Graph/cadence owner) with a cross-ref in `references/harness-seams.md` for the harness-node half; the sub-skills stay flow- and harness-blind. **It must never require the user to prompt "please make sure all phases/chores are represented" — that is the bug this requirement exists to kill.**
